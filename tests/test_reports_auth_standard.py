...
def test_mail_uses_reports_auth_contract_without_dedicated_sso():
    ...
    assert 'load_config_context "$API_DIR"' in remote
    assert 'AUTH_CONTEXT_URL=http://127.0.0.1:8001/auth/context' in (ROOT / 'apps/api/config/production/auth.env').read_text()
