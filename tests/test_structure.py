...
def test_required_structure():
    required = [
        ...
        'deploy/CONTRACT.md','deploy/core/load-env.sh','deploy/local/setup.sh','deploy/local/start.sh',
...
def test_ports_and_services_are_in_config_not_target():
    ...
    assert 'API_SYSTEMD_SERVICE=orbital-mail-api.service' in (ROOT/'apps/api/config/production/services.env').read_text()
